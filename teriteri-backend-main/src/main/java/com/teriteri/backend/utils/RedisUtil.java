...
public class RedisUtil {
    ...
    public <T> T getObject(String key, Class<T> clazz) {
        String objectString = (String) redisTemplate.opsForValue().get(key);//从redis获取指定键对应的值
        if (StringUtils.isNotBlank(objectString)) {//检查是否为空
            return JSONObject.parseObject(objectString, clazz);//反序列化成为对象
        }
        return null;
    }
